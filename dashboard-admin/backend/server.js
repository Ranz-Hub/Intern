...
app.put("/barang/:id", async (req, res) => {
  ...
  try {
    await db.query("UPDATE barang SET nama_barang = ?, harga = ?, stok = ? WHERE id = ?", 
      [nama_barang, harga, stok, id]
    );
    res.json({ message: "Barang berhasil diperbarui" });
  } catch (error) {
    res.status(500).json({ error: "Gagal memperbarui barang", detail: error });
  }
});
...
// Menjalankan server backend di port yang benar (5000 atau 3000)
app.listen(PORT, () => {
